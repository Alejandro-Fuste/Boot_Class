	...
	function onIntersection(imageEntities) {
		imageEntities.forEach((image) => {
			if (image.isIntersecting) {
				...
				image.target.src = image.target;
			}
		});
	}

	lazyImage.forEach((image) => observer.observe(image));
}

function loadImages() {
	fetch('/api/images').then((res) => res.json()).then((data) => createCards(data)).then(() => initLazyImages());
}
